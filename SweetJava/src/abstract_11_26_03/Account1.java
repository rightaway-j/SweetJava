...
//abstract키워드가 붙은 클래스를 추상 클래스(abstract class)라고한다.
//// 모든 클래스의 조상은 object클래스이다.
//// 다른 클래스로 부터 상속 받지 않는 모든 클래스들은 자동적으로 object클래스로 부터 상속 받느다.
 abstract class Account1 {// 컴파일러가 extends object를 추가한다.
...
//기본 생성자를컴파일러가 알아서 추가해줌
 ...
 int withdraw(int amount) throws Exception {// 인출한다.//final는 메소드 오버라이딩 금지(이것만은 안돼!-만든 의도와는 달리회손할까봐)
		if (balance < amount) {
